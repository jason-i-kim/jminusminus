...
public class Parser {
     ...
     * 
     * <pre>
     *   bitOrExpression ::= bitOrExpression // level 9
     *                                  {BITOR bitAndExpression}
     * </pre>
     * 
     * @return an AST for a conditionalExpression.
     */
    ...
    private JExpression bitOrExpression() {
        ...
        while (more) {
            if (have(BITOR)) {
                lhs = new JBitOrOp(line, lhs, bitXorExpression());
            } else {
                more = false;
            }
        }
   ...
   private JExpression equalityExpression() {                  //Level 6
    ...
    private JExpression relationalExpression() {
        int line = scanner.token().line();
        JExpression lhs = binshiftExpression();
        if (have(GT)) {
            return new JGreaterThanOp(line, lhs, binshiftExpression());
        } else if (have(LE)) {
            return new JLessEqualOp(line, lhs, binshiftExpression());
    ...
    /**
     * Parse a binshift expression.
     * 
     * <pre>
     * additiveExpression // level 4            --trg
	 *						{(BINLSHIFT | BINRSHIFT | LOGLSHIFT) additiveExpression} 	
     * </pre>
     * 
     * @return an AST for an binshiftExpression.
     */

    private JExpression binshiftExpression() {
        int line = scanner.token().line();
        boolean more = true;
        JExpression lhs = additiveExpression();
        while (more) {
            if (have(BINLSHIFT)) {
                lhs = new JBinLShiftOp(line, lhs, additiveExpression());
            } else if (have(BINRSHIFT)) {
                lhs = new JBinRShiftOp(line, lhs, additiveExpression());
            } else if (have(LOGRSHIFT)) {
                lhs = new JLogRShiftOp(line, lhs, additiveExpression());
            } else {
                more = false;
            }
        }

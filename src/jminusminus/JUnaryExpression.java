...
class JPromoteOp extends JUnaryExpression {
    ...
    /**
     * Construct an AST node for a promotion expression (unary plus) given its line number,
     * and the operand.
     * 
     * @param line
     *            line in which the promotion expression occurs in the source
     *            file.
     * @param arg
     *            the operand.	
     */
    ...
    /**
     * Analyzing the promotion operation involves analyzing its operand, checking
     * its type and determining the result type.
     * 
     * @param context
     *            context in which names are resolved.
    ...
    public JExpression analyze(Context context) {
        arg = arg.analyze(context);
//        arg.type().mustMatchExpected(line(), Type.INT);
        // TODO add byte and short to match test when they are implemented
    ...
    /**
     * Generating code for the promotion operation involves generating code for
     * the operand, and then the promotion instruction.
     * 
     * @param output
     *            the code emitter (basically an abstraction for producing the
     *            .class file).
